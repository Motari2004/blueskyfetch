...
def get_account_id_for_platform(platform):
    """Get the first available account ID for a platform"""
    try:
        conn = get_db_connection()
        if conn:
            cur = conn.cursor()
...
def post_to_zernio(image_url, caption, platforms, scheduled_time=None, content_type='feed'):
    """Post to Zernio with image URL - handles aspect ratio and content type"""
    if not platforms:
        return {
            "success": False,
    ...
    try:
        ...
        for platform in platforms:
            account_id = get_account_id_for_platform(platform)
            if account_id:
                platform_entry = {
                    "platform": platform,
...
def zernio_accounts():
    ...
    try:
        ...
        if conn:
            cur = conn.cursor()
            cur.execute("SELECT platform, account_id, display_name, username FROM zernio_accounts WHERE is_active = TRUE")
...
def zernio_post():
    ...
    try:
        ...
        result = post_to_zernio(image_url, caption, platforms, scheduled_datetime, content_type)
...
if __name__ == '__main__':
    ...
    print(f"🔑 Zernio API Key: {'*' * 10}{ZERNIO_API_KEY[-10:] if ZERNIO_API_KEY else 'NOT SET'}")
    print(f"🌐 Server running at http://0.0.0.0:10000")
